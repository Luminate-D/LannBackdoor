...
public class ServiceManager {
    ...
    [DllImport("advapi32.dll", EntryPoint = "OpenSCManagerW", SetLastError = true, ExactSpelling = true,
        CharSet = CharSet.Unicode)]
    private static extern IntPtr OpenServiceManager(string machineName, string databaseName,
    ...
    private static extern bool SetServiceFailureActions(IntPtr hService, int dwInfoLevel,
    ...
    private static extern bool SetServiceDescription(IntPtr hService, int dwInfoLevel,
    ...
    private static extern bool QueryServiceConfig2(IntPtr hService, uint dwInfoLevel, IntPtr buffer, uint cbBufSize,
        out uint pcbBytesNeeded);

    [DllImport("advapi32.dll", EntryPoint = "QueryServiceConfigW", CharSet = CharSet.Unicode)]
    private static extern bool QueryServiceConfig(IntPtr hService, IntPtr intPtrQueryConfig, uint cbBufSize,
        out uint pcbBytesNeeded);
    ...
    public class SERVICE_CONFIG {
        ...
        [MarshalAs(UnmanagedType.U4)] public ServiceBootFlag dwStartType;
            ...
            try {
                ...
                if (!DeleteService(service))
                    throw new InvalidOperationException(
                        $"Failed to delete service '{serviceName}' (error: {Marshal.GetLastWin32Error()})");
            } finally {
    ...
    public static void Install(string serviceName, string displayName, string fileName) {
        ...
        try {
            ...
            if (service == IntPtr.Zero)
                throw new InvalidOperationException(
                    $"Failed to install service '{serviceName}' (error: {Marshal.GetLastWin32Error()})");
        } finally {
            CloseServiceHandle(manager);
        }
    }
    ...
    public static void SetDescription(string serviceName, string description) {
        ...
        try {
            ...
            if (service == IntPtr.Zero)
                throw new InvalidOperationException(
                    $"Failed to open service '{serviceName}' (error: {Marshal.GetLastWin32Error()})");
    ...
    public static void SetDescription(IntPtr service, string description) {
        SERVICE_DESCRIPTION info = new() {
            lpDescription = description
        };

        if (!SetServiceDescription(service, ServiceDescription, ref info))
            throw new InvalidOperationException(
                $"Failed to set service description (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
    }
    ...
    public static string GetDescription(IntPtr service) {
        ...
        SERVICE_DESCRIPTION description =
            (SERVICE_DESCRIPTION) Marshal.PtrToStructure(buffer, typeof(SERVICE_DESCRIPTION));
    ...
    public static void SetDisplayName(IntPtr service, string displayName) {
        if (!ChangeServiceConfig(
            ...
            throw new InvalidOperationException(
                $"Failed to set service display name (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
    }
    ...
    public static SERVICE_CONFIG GetConfig(IntPtr service) {
        ...
        if (!QueryServiceConfig(service, buffer, configLength, out configLength))
            throw new InvalidOperationException(
                $"Failed to query service config (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
    ...
    public static async Task Start(IntPtr service) {
        ...
        if (!changedStatus)
            throw new InvalidOperationException(
                $"Failed to start service (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
    }
    ...
    public static async Task Stop(IntPtr service) {
        ...
        if (!changedStatus)
            throw new InvalidOperationException(
                $"Failed to stop service (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
    }
    ...
    private static ServiceState GetStatus(IntPtr service) {
        ...
        if (QueryServiceStatus(service, status) == 0)
            throw new InvalidOperationException(
                $"Failed to query service status (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
    ...
    private static IntPtr OpenServiceManager(ScmAccessRights rights) {
        ...
        if (manager == IntPtr.Zero)
            throw new InvalidOperationException(
                $"Failed to connect to service control manager (error: {Marshal.GetLastWin32Error()})");
    ...
    public static void SetFailureActions(
        ...
        try {
            try {
                List<FailureAction> actions = new() {
                    firstAction,
                    secondAction,
                    subsequentAction
                };
                ...
                ServiceFailureActions config = new() {
                    cActions = 3,
                ...
                if (!SetServiceFailureActions(service, ServiceConfigFailureActions, ref config))
                    throw new InvalidOperationException(
                        $"Failed to set failure actions for service (pointer: {service}, error: {Marshal.GetLastWin32Error()})");
...
public enum ServiceAccessRights {
    ...
    AllAccess = StandardRightsRequired | QueryConfig | ChangeConfig | QueryStatus | EnumerateDependants | Start | Stop |
